...
const App: React.FC = () => {
  ...
  const handleGitHubSync = useCallback(async (repoUrl: string) => {
    ...
    try {
        ...
        if (docs.length === 0) {
            ...
            // Note: We do NOT clear chat here anymore to preserve the welcome message or current context
            // unless we want to force a reset. Let's reset but keep welcome message.
            geminiService.reset();
             setChatState(prev => ({
                ...prev,
                // Keep existing messages if user has started chatting, or reset to welcome if it was just welcome
  ...
  return (
    <div className={`flex h-screen transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-900' : 'bg-gray-50'}`}>
      ...
      {/* Repository Sidebar (Mobile Overlay / Desktop Sidebar) */}
      <div 
      ...
      <div className="flex-1 flex flex-col h-full w-full relative">
        {/* Overlay when Repo is open on small screens */}
        {showRepo && (
          <div 
        ...
        {/* Header */}
        <header className="flex-none h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 lg:px-8 z-30 sticky top-0">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg overflow-hidden shadow-lg shadow-emerald-500/20">
        ...
        <main className="flex-1 overflow-y-auto relative scroll-smooth">
          <div className="max-w-4xl mx-auto px-4 py-8 min-h-full flex flex-col">
            ...
            {/* Note: Messages now start with Welcome Message, so this block is mostly for visual padding if chat is 'cleared' to just 1 message */}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-0 animate-[fadeIn_0.5s_ease-out_forwards] mb-8">
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">
                  {isSyncing ? (
                      <span className="flex items-center justify-center gap-2">
                       ...
                       <>Synced with GitHub Repo: <span className="font-semibold text-blue-600 dark:text-blue-400">{githubRepoUrl}</span>.</>
                  )}
                </p>
                ...
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl transition-opacity duration-500 ${isSyncing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                  {[
                    ...
                    "Summarize the latest public transport changes."
                  ].map((prompt, idx) => (
                    <button
                      key={idx}
